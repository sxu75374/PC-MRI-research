...
for i = 1:60:length(filelist)
    c1 = c1+1;
    for j = 1:20
        path_complex = filelist{i+j-1,1};
        info_complex{c1,j} = dicominfo(path_complex);
        data_complex(:,:,c1,j) = double(dicomread(path_complex));
    end
end
...
for i3 = 41:60:length(filelist)
    c3 = c3+1;
    for l = 1:20
        path_p = filelist{i3+l-1,1};
%         info_p{c3,l} = dicominfo(path_p);
%         data_p(:,:,c3,l) = double(dicomread(path_p));
        temp1 = dicominfo(path_p);
        info_p{c3,l} = temp1;
        temp2 = double(dicomread(path_p));
        data_p(:,:,c3,l) = temp2 * temp1.RescaleSlope + temp1.RescaleIntercept;
    end
end
end
